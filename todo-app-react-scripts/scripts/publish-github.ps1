# GitHub Release Auto Publish Script
# Usage: powershell -ExecutionPolicy Bypass -File scripts\publish-github.ps1 [version]
...
$releaseTitle = "TimelineFlow v$version"
...
Write-Host "  GitHub Release Publish Tool" -ForegroundColor Cyan
...
try {
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/$owner/$repo/releases/tags/$tagName" -Method Get -Headers $headers
...
# 3. Upload files
Write-Host ""
Write-Host "[4/6] Uploading files to GitHub..." -ForegroundColor Yellow

# Upload EXE
Write-Host "   Uploading EXE file..." -ForegroundColor Gray
$exeUploadUrl = $uploadUrl + "?name=todo-app-setup-$version.exe"
...
$blockmapUploadUrl = $uploadUrl + "?name=todo-app-setup-$version.exe.blockmap"
$blockmapBytes = [System.IO.File]::ReadAllBytes($blockmapPath)
$blockmapHeaders = @{
    'Authorization' = "Bearer $token"
    'Content-Type' = 'application/octet-stream'
}
try {
    Invoke-RestMethod -Uri $blockmapUploadUrl -Method Post -Headers $blockmapHeaders -Body $blockmapBytes | Out-Null
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  OK GitHub Release completed!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  https://github.com/$owner/$repo/releases/download/$tagName/todo-app-setup-$version.exe" -ForegroundColor Cyan
Write-Host ""
